    ...
    m_pTblWidgetEmail(NULL)
{
    ...
    // 添加数据
    vector<service_meta> vecEmails;
    vecEmails = global_cloudnodes();
    for (int i = 0; i < vecEmails.size(); ++i)
    {
        addEmailtoWidget(QString::fromLocal8Bit(vecEmails[i].user.c_str()),
                         QString::fromLocal8Bit(vecEmails[i].pawd.c_str()),
                         QString::fromLocal8Bit(vecEmails[i].smtp.c_str()),
                         QString::fromLocal8Bit(vecEmails[i].pop3.c_str()) );
    }
